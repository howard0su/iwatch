...
static void OnDrawTime(tContext *pContext)
{
  ...
  GrContextFontSet(pContext, (const tFont*)&g_sFontRonda25b); 
#else  
  ...
  {
#if defined(PRODUCT_W002) || defined(PRODUCT_W004)  
	window_button(pContext, KEY_EXIT|0x80, "Exit"); 
#else  	
...
static void OnDrawDate(tContext *pContext)
{
  char buf[20];
#if defined(PRODUCT_W002) || defined(PRODUCT_W004)  
 GrContextFontSet(pContext, &g_sFontHelvetic28);
#else  
...
#if defined(PRODUCT_W002) || defined(PRODUCT_W004)  
  	window_volume(pContext, 56, 137, 8, level);
	window_button(pContext, KEY_ENTER|0x80, "OK");
#else
    	// clear the region
  	GrContextForegroundSet(pContext, ClrBlack);
  ...
  case EVENT_WINDOW_PAINT:
    {
  	...
  	GrRectFill((tContext*)rparam, &client_clip);

  	GrContextForegroundSet((tContext*)rparam, ClrWhite);
  		
	GrContextFontSet((tContext*)rparam, (tFont*)&g_sFontExIcons32);
	GrStringDraw((tContext*)rparam, "e", 1, 1, 68, 0);
#endif    	
...
uint8_t configvol_process(uint8_t event, uint16_t lparam, void* rparam)
{
  ...
  case EVENT_WINDOW_PAINT:
	...
	GrContextFontSet((tContext*)rparam, (tFont*)&g_sFontExIcons32);
	GrStringDraw((tContext*)rparam,"f", 1, 1, 68, 0);
#endif     	
