...
#if defined(PRODUCT_W002) || defined(PRODUCT_W004)  
static const tRectangle region_Layout[] = 
{
  {0,   0, LCD_WIDTH, 45},
  {0,  46, LCD_WIDTH, 100 },
  {0, 101, LCD_WIDTH, 135},
  {0, 136, LCD_WIDTH, LCD_Y_SIZE},	
};
static const tRectangle region_3grid[] =
{
  {0, 0, LCD_WIDTH, 45},
  {0, 46, 72, LCD_Y_SIZE},
  {73, 46, LCD_WIDTH, LCD_Y_SIZE}
};
...
static const tRectangle region_4grid[] =
{
  { 0,  0, LCD_WIDTH, 45},
  { 0, 46, 44, LCD_Y_SIZE},
  {45, 46, 102, LCD_Y_SIZE},
  {103,46, LCD_WIDTH, LCD_Y_SIZE},
};
...
static const tRectangle region_5grid[] =
{
  {0, 0, LCD_WIDTH, 45},  
  {0, 73, 44, LCD_Y_SIZE},
  {45, 73, 102, LCD_Y_SIZE},
  {103,73, LCD_WIDTH, LCD_Y_SIZE},
  {0, 46, LCD_WIDTH, 72},
};
...
static void drawGridTime(tContext *pContext)
{
  ...
  GrContextForegroundSet(pContext, ClrBlack);
    	...
    	GrStringDrawCentered(pContext, buf, -1, LCD_WIDTH/2, 20, 0);
#else  
...
static void drawGridData(tContext *pContext, uint8_t grid, uint32_t data)
{
  ...
  case GRID_3:
	...
	if(buf2!=NULL)
	{
    		GrStringDrawCentered(pContext, buf1, -1, region_3grid[index].sXMin + (region_3grid[index].sXMax - region_3grid[index].sXMin) / 2, region_3grid[index].sYMin + 17,0);
    		GrStringDrawCentered(pContext, buf2, -1, region_3grid[index].sXMin + (region_3grid[index].sXMax - region_3grid[index].sXMin) / 2, region_3grid[index].sYMin + 34,0);		
	}
	else		
		GrStringDrawCentered(pContext, d->name, -1, region_3grid[index].sXMin + (region_3grid[index].sXMax - region_3grid[index].sXMin) / 2, region_3grid[index].sYMin+24, 0);
	
	GrContextForegroundSet(pContext, ClrBlack);
	GrContextFontSet(pContext, &g_sFontBaby16b);
	GrStringDrawCentered(pContext, buf, -1,  region_3grid[index].sXMin + 36, region_3grid[index].sYMin+72, 0);
	GrContextForegroundSet(pContext, ClrWhite);
    	if (unit)
    	{
      		GrContextFontSet(pContext, &g_sFontBaby16);
      		GrStringDrawCentered(pContext, unit, -1, region_3grid[index].sXMin + 36, region_3grid[index].sYMin + 106, 0);
    	}	
#else  	
    GrContextFontSet(pContext, &g_sFontGothic18);
  ...
  case GRID_4:
    	...
    	if(buf2!=NULL)
    	{
    		GrStringDrawCentered(pContext, buf1, -1, region_4grid[index].sXMin + (region_4grid[index].sXMax - region_4grid[index].sXMin) / 2, region_4grid[index].sYMin + 17,0);
    		GrStringDrawCentered(pContext, buf2, -1, region_4grid[index].sXMin + (region_4grid[index].sXMax - region_4grid[index].sXMin) / 2, region_4grid[index].sYMin + 34,0);
    	}
    	else    	
    	{		
    		GrStringDrawCentered(pContext, d->name, -1, region_4grid[index].sXMin + (region_4grid[index].sXMax - region_4grid[index].sXMin) / 2, region_4grid[index].sYMin + 24,0);
    	}
    		
	GrContextForegroundSet(pContext, ClrBlack);
    	GrContextFontSet(pContext, &g_sFontBaby16b);    
    	GrStringDrawCentered(pContext, buf, -1, region_4grid[index].sXMin + (region_4grid[index].sXMax - region_4grid[index].sXMin) / 2, region_4grid[index].sYMin + 72, 0);
    	GrContextForegroundSet(pContext, ClrWhite);
    	if (unit)
    	{
      		GrContextFontSet(pContext, &g_sFontBaby16);    
      		GrStringDrawCentered(pContext, unit, -1, region_4grid[index].sXMin + (region_4grid[index].sXMax - region_4grid[index].sXMin) / 2, region_4grid[index].sYMin + 106, 0);
    	}
#else  	
    GrContextFontSet(pContext, &g_sFontGothic18);
  ...
  case GRID_5:
    	...
    	{	
    		...
    		GrStringDraw(pContext, buf, -1, region_5grid[index].sXMin + 24 , region_5grid[index].sYMin + 4, 0);   	
    		if (unit)
    		{
      			GrContextFontSet(pContext, &g_sFontHelvetic20);
      			GrStringDraw(pContext, unit, -1, region_5grid[index].sXMin + 100, region_5grid[index].sYMin + 4, 0);
    		}    		
    	...
    	{	
    		...
    		if(buf2!=NULL)
    		{
    			GrStringDrawCentered(pContext, buf1, -1, region_5grid[index].sXMin + (region_5grid[index].sXMax - region_5grid[index].sXMin) / 2, region_5grid[index].sYMin + 7,0);
    			GrStringDrawCentered(pContext, buf2, -1, region_5grid[index].sXMin + (region_5grid[index].sXMax - region_5grid[index].sXMin) / 2, region_5grid[index].sYMin + 19,0);
    		}
    		else    	
    			GrStringDrawCentered(pContext, d->name, -1, region_5grid[index].sXMin + (region_5grid[index].sXMax - region_5grid[index].sXMin) / 2 , region_5grid[index].sYMin + 14, 0);
		
		GrContextForegroundSet(pContext, ClrBlack);
    		GrContextFontSet(pContext, &g_sFontBaby16);    		
    		GrStringDrawCentered(pContext, buf, -1, region_5grid[index].sXMin + (region_5grid[index].sXMax - region_5grid[index].sXMin) / 2 , region_5grid[index].sYMin + 45, 0);
    		GrContextForegroundSet(pContext, ClrWhite);
    		if (unit)
    		{
      			GrContextFontSet(pContext, &g_sFontBaby16);
      			GrStringDrawCentered(pContext, unit, -1, region_5grid[index].sXMin + (region_5grid[index].sXMax - region_5grid[index].sXMin) / 2 , region_5grid[index].sYMin + 79, 0);
    		}    		
    	}	
...
#if defined(PRODUCT_W002) || defined(PRODUCT_W004)  
	GrContextForegroundSet(pContext, ClrWhite);
	GrRectFill(pContext, &region_Layout[0]);
	GrRectFill(pContext, &region_Layout[2]);		
#else
  GrContextForegroundSet(pContext, ClrWhite);
  switch(window_readconfig()->sports_grid)
  {
