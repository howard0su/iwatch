...
#if defined(PRODUCT_W002) || defined(PRODUCT_W004)  
	GrContextFontSet(pContext, (tFont*)&g_sFontRonda25b);
#else  
...
#if defined(PRODUCT_W002) || defined(PRODUCT_W004)  
	GrLineDrawH(pContext, 0, LCD_WIDTH, (i - topView) * 20 + NUMBASE + 18);
#else      
      GrLineDrawH(pContext, 0, LCD_WIDTH, (i - topView) * 20 + NUMBASE + 15);
#endif      
    }
#if defined(PRODUCT_W002) || defined(PRODUCT_W004)  
	GrContextFontSet(pContext, (tFont*)&g_sFontRonda10);
#else
    GrContextFontSet(pContext, &g_sFontGothic18b);
#endif    
    for(int i = topView; (i < topView + 3) && (i < currentStop); i++)
...
#if defined(PRODUCT_W002) || defined(PRODUCT_W004)  
	GrStringDraw(pContext, buf, -1, 70, (i - topView) * 20 + NUMBASE +  4, 0);
#else        
...
#if defined(PRODUCT_W002) || defined(PRODUCT_W004)  
	GrStringDraw(pContext, buf, -1, 90, (i - topView) * 20 + NUMBASE + 4, 0);
#else        
